...
```python
from motion_planner import selected_plan_to_motion_requests
...
```python
from motion_planner import attached_object_transform_from_state
...
```powershell
python -m pip install -e "Motion_Planner[vlm]"
...
python -m pytest Motion_Planner\tests\test_vlm_provider_live.py -q
```
...
```bash
python -m pytest Motion_Planner/tests Task_Planner/tests -q
```
