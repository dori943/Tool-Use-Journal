...
Planner A intentionally emits a partial-order DAG, while Task Planner uses a
...
def build_request_from_current_planner_a(
    ...
    *,
    ...
    """Build a Task-Planner request from a current Planner-A result.
...
def adapt_current_planner_a_output(
    ...
    if robot_state.get("in_hand") is not None:
        raise ValueError(
            ...
            "provide a normalized Task-Planner initial_state for held resources"
        )
