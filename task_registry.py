...
TASKS = {
    "c1_1": TaskEnv(
        "c1_1_lego_sweep",
        "C1_1_LegoSweep",
        # 0911: "도구를 골라"가 별개 목표로 읽혀 국자/포크/가위를 EE 랙에 넣는
        # 서브골이 생겼다. 도구 선택은 시스템이 하는 일이므로 지시문에서 뺀다.
    ...
    "c3_2": TaskEnv(
        ...
        instruction=(
            ...
            "각 트레이에 접시, 머그컵, 포크, 숟가락을 담고, 빵과 과일은 접시 위에 올려라. 두 트레이를 같은 구성으로, 식사하기 편하도록 식기를 가지런히 배치하라."
        ),
    ),
