...
@pytest.mark.parametrize('attachment, frozen', [
    (None, False),
...
def test_retention_feedback_depends_on_actual_object_constraint(attachment, frozen):
    env = object()
    ...
    expected = [.1, .1, .1] if frozen else [.1, .094, .106]
