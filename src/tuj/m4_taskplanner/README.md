...
Task Planner(M4) 모듈 위치는 `src/tuj/m4_taskplanner/`이며, 패키지 경로는
`tuj.m4_taskplanner`다. (파이프라인 재정의로 모듈 번호를 하나씩 밀어 단계명과
패키지 경로를 일치시켰다.)
...
`roles.selected_tool`에서 읽고 결과의 `candidate_assignments`로 전달한다. M2의
`?tool` binding도 같은 ID로 치환되므로 `PICK_TOOL`, 작업, 운반, `RETURN_TOOL` 모두
동일한 구체 Tool ID를 M5에 전달한다. M2에 acquire/place detail이 이미 있으면 이를
...
python -m pytest src/tuj/m4_taskplanner/tests -q
```
...
```bash
PYTHONPATH=src python -m tuj.m4_taskplanner.cli --help
```
...
```powershell
$env:PYTHONPATH = "src"
python -m tuj.m4_taskplanner.cli --help
```

`GK + M2`을 필수 입력으로 받아 다음을 하나의 Dijkstra 검색으로 계획한다.
...
기존 M2 action/DAG 형식의 `tool_id`, `selected_tool_id`, `selected_tool`도 호환
...
```bash
PYTHONPATH=src python -m tuj.m4_taskplanner.cli plan \
...
앞단에서 확정한 `roles.selected_tool`을 제공하고, `m2.json`은 scene의 `nodes/edges`를
제공한다. 이전 `m2_subgoals` 형식도 계속 지원하며, scene graph가 별도 파일이면
`--m1`로 명시할 수 있다. `--robot-spec`은 EE payload/capability를 보강한다.
...
PYTHONPATH=src python -m tuj.m4_taskplanner.cli plan \
  ...
  --m2 output/c1_1/m2.json \
  --robot-spec configs/robot_spec.json \
  --initial-state src/tuj/m4_taskplanner/examples/initial_state.json \
  --output output/c1_1/task_planner.json
```

`--initial-state`는 `InitialState` 스키마를 그대로 사용하며 robot spec의 초기 상태보다
우선한다. 예시는 `src/tuj/m4_taskplanner/examples/initial_state.json`에 있다.

재계획:

```bash
PYTHONPATH=src python -m tuj.m4_taskplanner.cli replan \
...
테스트는 입력 검증, M2 DAG 제약, GK/M2 변환, ID 정규화, group EE binding,
