"""Integration coverage for the GK + M2 input boundary."""
...
from tuj.m4_taskplanner.cli import main
...
from tuj.m4_taskplanner.models import Condition, InitialState
...
def _m2(*, tool_id: str = "obj_plate_light_plate") -> dict:
    # Uses the lower-case class prefix found in the repository's m2.json.
    return {
...
def _bundle_gk_and_scene_m2() -> tuple[dict, dict]:
    """Mirror the attached gk_bundle + scene-graph M2 contract."""

    gk = deepcopy(_gk())
    rough = deepcopy(_m2()["m2_subgoals"][0])
    ...
    record.update(
        {
            ...
            "partial_order": _m2()["m2_partial_order"],
        }
    )
    return gk, _m1()
...
def test_gk_adapter_normalizes_c1_1_ids_actions_and_conditions() -> None:
    request = build_request_from_gk(
        _gk(), _m2(), m1_payload=_m1(), robot_spec_payload=_robot_spec()
    )

    first, sweep = request.task_graph.subgoals
    assert request.task_graph.log["adapter"] == "gk-m2-v1"
...
def test_gk_bundle_uses_selected_tool_and_m2_scene_graph() -> None:
    gk, scene_m2 = _bundle_gk_and_scene_m2()
    ...
    assert request.task_graph.log["adapter"] == "gk-bundle+m2-scene-v1"
...
def test_repository_c1_1_bundle_selects_forwarded_light_plate() -> None:
    ...
    request = build_request_from_gk(
        ...
        json.loads((output / "m2.json").read_text(encoding="utf-8")),
...
def test_gk_does_not_fallback_from_upstream_selected_tool() -> None:
    request = build_request_from_gk(
        _gk(),
        _m2(tool_id="obj_plate_heavy_plate"),
        m1_payload=_m1(),
        robot_spec_payload=_robot_spec(),
    )
...
def test_gk_requires_explicit_initial_robot_state() -> None:
    ...
    with pytest.raises(ValueError, match="initial current_ee is missing"):
        build_request_from_gk(
            _gk(), _m2(), m1_payload=_m1(), robot_spec_payload=robot_spec
        )
...
def test_gk_without_m2_fails_with_actionable_error() -> None:
