...
from tuj.m4_motion.pipeline import (  # noqa: E402
    MotionPlanningPipeline,
    MotionPlanningPipelineError,
)
from tuj.m4_motion.push_to_region import (  # noqa: E402
...
def _retarget_equivalent_pick_keyframes(
    ...
    *,
    ...
    M3 to select another instance of the same plate asset.  The VLM's symbolic
...
def main() -> int:
    ...
    try:
        ...
        if not isinstance(selected_ee_spec, dict) or not isinstance(
            ...
            raise RuntimeError(
                f"M0 robot spec is missing grip_force_n for {pick_binding.ee}"
            )
        ...
        if (
            ...
            raise RuntimeError(
                f"M0 robot spec is missing fingerpad_friction for "
                f"{pick_binding.ee}"
            )
        ...
        if not isinstance(full_size_mm, (list, tuple)) or len(full_size_mm) != 3:
            raise RuntimeError(
                f"M0 tool metadata is missing full_size_mm for {pick_binding.tool}"
            )
        ...
        if (
            ...
            raise RuntimeError(
                f"M0 tool metadata is missing mass/friction for {pick_binding.tool}"
            )
        ...
        if args.sweep_target_ids:
            ...
            if unknown_targets:
                raise RuntimeError(
                    "staged sweep targets are absent from the selected M3 "
