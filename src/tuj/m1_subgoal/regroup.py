# -*- coding: utf-8 -*-
"""M1-6 — 측정 반영 후 서브골 분할 (0828 결정).

batch 질의 응답의 partition(그룹 구성)대로, 서브골 하나를 그룹별 서브골 여러 개로
나눈다. apply_m2(판정·객체 확정)가 partition_plan을 채운 뒤 호출한다.
        ...
        if k == len(part):
            ...
            for tj in tstrs:             # 그룹별 사전조건 — 각 sweep의 establish와 문자열 일치
                d2b["pre"].append({"id": f"{csid}_d2b_p{len(d2b['pre'])}",
                                   "expr": f"in({tj}, {r})", "head": "in", "eval_by": "m1"})
            details.append(d2b)
...
def split_after_m2(m1_out: dict) -> list[str]:
    ...
    m1_out["m1_partial_order"], m1_out["m1_mutex"] = edges, mutex
    m1_out["m1_invariants"] = invariants_for(new_subs)
    ...
    queries = [q for q in m1_out.get("m1_queries", []) if q["subgoal_id"] in kept_ids]
    for s2 in new_subs:
        if "split_from" in s2:
            queries += build_queries(s2, s2["details"])
    m1_out["m1_queries"] = queries
    st = m1_out.setdefault("m1_stats", {})
    st.update({"n_subgoals": len(new_subs), "n_details": len(all_details),
               "n_edges": len(edges), "n_mutex": len(mutex),
               "n_m2_queries": len(queries),
