"""EE-conditioned 판정 (구 M3-(c) — M1 으로 통합): intrinsic 수치 × 로봇 스펙 → EE별 {feasible, margin, reason}.
...
def evaluate_ee(ee_spec: dict, intrinsic: dict) -> dict:
    ...
    if t in ("parallel_2f", "underactuated_3f"):
        ...
        if "seal_patch_rms_mm" in g:              # 필드 없는 옛 기하는 판정 보류(오판 방지)
            patch_rms = g["seal_patch_rms_mm"]
