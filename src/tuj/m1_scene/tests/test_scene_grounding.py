...
def _props(density=1000.0, extents=(100.0, 50.0, 40.0), material="plastic"):
    return {
        "geometry": {
            "length_mm": 100.0, "diameter_mm": None,
            "extents_mm": list(extents), "height_z_mm": 100.0,
            "cylinder_like": False, "surface_rms_mm": 1.0,
            "footprint_mm": [100.0, 50.0], "height_mm": 40.0,
            "seal_patch_rms_mm": 1.0,
        },
        "material": material, "density_kgm3": density, "mass_kg": 0.2,
        "youngs_gpa": 2.0,
        "mu": {"mu": 0.4, "stage": 0, "material": material, "rms_mm": 1.0},
        "confidence": 0.8,
        "materials_topk": [{"name": material, "prob": 0.8}],
    }
...
def _spoon_scene(center=(0, 0, 10), half=(50.0, 25.0, 20.0)):
    scene = build_m1([{"name": "spoon", "cls": "spoon", "points": _points(center, half)}])
    # Stable bbox for retrieval tests (orientation-invariant vs stored extents).
...
def test_ground_scene_populates_all_scene_nodes_and_reuses_memory(tmp_path):
    ...
    stats = ground_scene(first, backend=MockBackend(),
                         memory=PropertyMemory(memory_path, task_id="t1"),
...
def test_same_task_exact_hit_skips_c3_and_full(tmp_path):
    ...
    memory.update_entry("same", spoon_id, _props())
    memory.save()
...
def test_cross_task_hit_calls_c3_once_skips_full(tmp_path):
    memory = PropertyMemory(tmp_path / "m.json", task_id="now",
                            bbox_relative_threshold=0.25,
                            density_relative_threshold=0.20)
    memory.update_entry("past", "obj_spoon_spoon", _props(1000))
    memory.save()
    ...
    def infer(_):
        ...
        return DensityOnlyResult(
            1050, True, {"total_tokens": 42},
            materials_topk=[{"name": "plastic", "prob": 1.0, "density_kgm3": [1000, 1100]}],
            material_committed=True, top1_gap=0.5)

    debug = {}
    stats = ground_scene(
        _spoon_scene(), backend=full, memory=memory, density_infer=infer,
        retrieval_debug=debug)
    assert c3["calls"] == 1 and full.calls == 0
    assert stats["memory_hits"] == 1 and stats["grounded"] == 0
    d = debug["obj_spoon_spoon"]
    assert d["lookup_type"] == "cross_task" and d["result"] == "HIT"
...
def test_cross_task_density_miss_runs_full_and_stores(tmp_path):
    ...
    memory = PropertyMemory(path, task_id="later",
                            density_relative_threshold=0.20)
    memory.update_entry("past", "obj_spoon_spoon", _props(1000))
    memory.save()
    ...
    stats = ground_scene(
        _spoon_scene(), backend=full, memory=memory, density_infer=infer,
        retrieval_debug=debug, source="mock")
    assert c3["calls"] == 1 and full.calls == 1
    assert stats["grounded"] == 1 and stats["memory_hits"] == 0
    assert debug["obj_spoon_spoon"]["miss_reason"] == "DENSITY_THRESHOLD_EXCEEDED"
    assert debug["obj_spoon_spoon"]["full_m3_called"] is True
    reloaded = PropertyMemory(path, task_id="later")
    entry, _, _ = reloaded.lookup_exact("later", "obj_spoon_spoon")
    assert entry is not None


def test_no_bbox_candidate_skips_c3_runs_full(tmp_path):
    memory = PropertyMemory(tmp_path / "m.json", task_id="now")
    memory.update_entry("past", "obj_spoon_spoon", _props(extents=(300, 200, 150)))
    memory.save()
    ...
    stats = ground_scene(
        _spoon_scene(), backend=full, memory=memory, density_infer=infer,
        retrieval_debug=debug)
    assert c3["calls"] == 0 and full.calls == 1
    assert stats["grounded"] == 1
    assert debug["obj_spoon_spoon"]["miss_reason"] == "NO_BBOX_CANDIDATE"


def test_numpy_bbox_and_extents_filter(tmp_path):
    m = ObjectKnowledgeManager(tmp_path / "m.json", bbox_relative_threshold=0.25)
    props = _props(extents=(100.0, 50.0, 40.0))
    props["geometry"]["extents_mm"] = np.array([100.0, 50.0, 40.0])
    m.update("past", "a", props)
    found = m.filter_bbox("now", np.array([102.0, 51.0, 39.0]))
    assert len(found) == 1
    assert found[0]["object_id"] == "a"
    assert _as_size3(np.array([1.0, 2.0, 3.0])) == [1.0, 2.0, 3.0]
...
def test_failure_recovery_preserved_after_m1_ground(tmp_path):
    ...
    memory = PropertyMemory(path, task_id="t1")
    ground_scene(_spoon_scene(), backend=MockBackend(), memory=memory, source="mock")
    saved = json.loads(path.read_text(encoding="utf-8"))
    assert saved["failure_recovery_experience"]["experiences"] == [{"id": "failure-1"}]
    assert len(saved["object_knowledge"]["objects"]) == 1
...
def test_stage_m1_argv_forwards_memory_and_thresholds():
    ...
    spec = importlib.util.spec_from_file_location("tuj_scripts_run", path)
    run_mod = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    spec.loader.exec_module(run_mod)

    args = SimpleNamespace(
        seed=0, backend="siphy", model="gpt-4o",
        memory=r"output\m0_m1_test_memory.json",
        m0_bbox_threshold=0.25, m0_density_threshold=0.20, view=False)
    argv = run_mod._stage_m1_argv("c2_1", Path("output/c2_1"), args)
    assert "--memory" in argv
    assert argv[argv.index("--memory") + 1] == r"output\m0_m1_test_memory.json"
    assert argv[argv.index("--model") + 1] == "gpt-4o"
    assert argv[argv.index("--backend") + 1] == "siphy"
    assert float(argv[argv.index("--m0-bbox-threshold") + 1]) == 0.25
    assert float(argv[argv.index("--m0-density-threshold") + 1]) == 0.20


def _vac_ee_pool():
    return [{"ee_id": "VAC", "type": "vacuum", "payload_kg": 5.0,
             "seal_rms_tol_mm": 1.5, "seal_diameter_mm": 30.0}]


def _hit_with_bad_surface_rms(tmp_path, bad_rms):
    """same-task HIT with invalid surface_rms → geometry refresh, no Full SiPhy."""
    from tuj.m1_scene.grounding import geometry_is_current

    spoon_id = "obj_spoon_spoon"
    props = _props(material="apple_flesh", density=900.0)
    props["geometry"]["surface_rms_mm"] = bad_rms
    props["mu"]["rms_mm"] = bad_rms
    assert not geometry_is_current(props["geometry"])

    memory = PropertyMemory(tmp_path / "m.json", task_id="same")
    memory.update_entry("same", spoon_id, props)
    memory.save()
    ...
    stats = ground_scene(
        scene, backend=full, memory=memory, density_infer=infer,
        ee_pool=_vac_ee_pool(), reach_mm=850)
    node = scene["nodes"][0]
    assert stats["memory_hits"] == 1 and stats["grounded"] == 0
    ...
    assert geometry_is_current(node["geometry"])
    assert np.isfinite(node["geometry"]["surface_rms_mm"])
    assert "VAC" in node["ee"]
    assert isinstance(node["ee"]["VAC"]["feasible"], bool)


def test_memory_hit_refreshes_none_surface_rms_without_full_siphy(tmp_path):
    _hit_with_bad_surface_rms(tmp_path, None)


def test_memory_hit_refreshes_nan_surface_rms_without_full_siphy(tmp_path):
    _hit_with_bad_surface_rms(tmp_path, float("nan"))
