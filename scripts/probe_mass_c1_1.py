...
★ 현재 비활성 (M4 프리미티브 연결 전) — 실행 시 안내만 출력하고 종료.
  xfrc 직접 인가 방식은 sim 검증용 거친 버전이라, M4의 grasp-lift-replace
  (src/tuj/m4_motion/README.md) 완성 시 lift_probe()를 교체하고
  아래 main() 끝의 주석을 해제해 활성화한다.

배치: Tool-Use-Journal/scripts/probe_mass_c1_1.py
선행: python scripts/run_m2_c1_1.py --backend siphy   (시각 추정 생성)
실행: python scripts/probe_mass_c1_1.py               (기본: light/heavy 접시 프로브)
      python scripts/probe_mass_c1_1.py --update      (m2_intrinsic.json에 프로브 질량 반영)
...
트리거 논의: heavy 접시는 시각 추정이 '자신 있게 틀려' margin 게이트가 안 울림.
  ...
  사전검증 정책 ③ M5 실행 실패 피드백 — 본 스크립트는 그 트리거가 울렸다고 가정한 2단.
"""
...
def lift_probe(env, root_body: str, settle=100, steps=120):
    """sim에서 리프트 프로브 실행. (실제 로봇에선 M4의 제어된 리프트가 이 역할)"""
    sim = env.sim
...
def main():
    ...
    ap.add_argument("--update", action="store_true",
                    help="프로브 질량을 m2_intrinsic.json에 반영 (stage 2 기록)")
    args = ap.parse_args()
    ...
    from tuj.m2_grounding import evaluate_ee

    cache_p = OUT / "m2_intrinsic.json"
    ...
    if args.update and est_cache:
        ...
        print(f"[update] m2_intrinsic.json에 프로브 질량 반영 (stage 2) — "
              f"eval_m2_mass_c1_1.py 재실행으로 decision_agreement 확인")


if __name__ == "__main__":
    # TODO(M4): grasp-lift-replace 프리미티브 연결 시 아래 주석 해제
    print("[probe] 질량 프로브는 M4 프리미티브(probe_lift) 연결 전까지 비활성 상태입니다.")
