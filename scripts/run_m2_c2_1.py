...
def main():
    ...
    ap.add_argument("--memory", default=str(ROOT / "output" / "memory.json"),
                    help="지속 물성 메모리 경로 ('none'이면 비활성)")
    args = ap.parse_args()
    ...
    memory = None if args.memory == "none" else PropertyMemory(args.memory)
    ...
    if memory is not None:
        stats = memory.update(mat._cache, source=args.backend)
        memory.save()
        print(f"[M2] memory update: new={stats['new']} upgraded={stats['upgraded']} "
              f"kept={stats['kept']} -> {args.memory}")
    print(f"\n[M2] tier-2 접지 {n_t2}/{len(m0['nodes'])} 객체 (lazy, preload {preloaded}건 포함),"
