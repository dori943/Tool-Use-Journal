# -*- coding: utf-8 -*-
"""confidence 실험 — M2 접지 전후 LLM 자기보고 신뢰도 변화 측정.

목적: "측정값 없이 내린 계획"과 "M2 측정값을 반영한 계획"에 대해 LLM이 스스로
...
전제: output/<task>/m0.json 과 m2.json 이 있어야 한다. OPENAI_API_KEY 필수.
...
WITH_GROUNDING = """M2 측정 모듈이 계획의 조건들을 실측한 결과가 도착했다:
{summary}
...
def plan_summary(out: dict) -> str:
    ...
    lines.append(f"- 실행 순서 제약 {len(out['m1_partial_order'])}개, "
                 f"M2 측정 요청 {len(out['m1_queries'])}건")
    return "\n".join(lines)


def m2_summary(out: dict) -> str:
...
def main():
    ...
    with open(os.path.join(tdir, "m0.json"), encoding="utf-8") as f:
        m0 = json.load(f)
    ...
    results = []
    for i in range(runs):
        out = run_m1(task, m0, rough=rough)            # 분해 + 객체 선택 (LLM 2회)
        ...
        apply_m2(out, responses)                        # M2 응답 반영 + 도구 확정
        after = ask_confidence(client, model, task, plan_summary(out),
                               WITH_GROUNDING.format(summary=m2_summary(out)))
