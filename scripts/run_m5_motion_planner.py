"""Run the M5 Motion Planner for C1_1 with safe repository defaults.
...
The runner forwards unknown options to ``c1_1_openai_motion_run.py``.  The
selected end-effector, tool, targets, and goal are read only from the M4 Task
Planner result.
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("[M5] " + shlex.join(command), flush=True)
    if args.dry_run:
        return 0
    try:
