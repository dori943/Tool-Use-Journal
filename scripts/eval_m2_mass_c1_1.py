"""M2 질량 추정 품질 평가 — 추정치(m2_intrinsic.json) vs sim GT(subtree mass).

배치: Tool-Use-Journal/scripts/eval_m2_mass_c1_1.py
선행: python scripts/run_m2_c1_1.py --backend siphy   (평가 대상 추정 생성)
실행: python scripts/eval_m2_mass_c1_1.py
...
def main():
    ...
    if not cache_p.exists():
        sys.exit("[err] m2_intrinsic.json 없음 — 먼저 run_m2_c1_1.py --backend siphy 실행")
