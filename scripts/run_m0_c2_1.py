"""C2-1 씬에서 M0만 실행 → m0.json + m0_points.npz 산출.

배치: Tool-Use-Journal/scripts/run_m0_c2_1.py
실행: python scripts/run_m0_c2_1.py            (레포 루트에서)
출력: output/c2_1/m0.json          — bbox 노드 + coarse 관계 (M1 전달용, VLM 0회)
      output/c2_1/m0_points.npz   — 노드별 점군 (M2 접지 입력, run_m2가 로드)
      output/c2_1/crops/*.png     — 노드별 마스크 크롭 (SiPhy 백엔드 VLM 입력)
...
def class_of(inst: str) -> str | None:
    # C2-1 분류 태스크: 대상 5종 + 3색 트레이(컨테이너) + rack
    ...
    if inst.endswith("_tray"):
        return "tray"                                 # green/blue/red_tray (분류 목적지)
    if "rack" in inst.lower():
        return "rack"                                 # tool_rest 별칭 해결용
    return None
...
def object_bound_points(env):
    """추적 대상(대상 5종 + 3색 트레이)의 월드 바운드 점. EE 랙·로봇 제외."""
    m, d = env.sim.model, env.sim.data
    gids = set()
    for o in env.target_objects + env.trays:
...
def main():
    env = suite.make(
        env_name="C2_1_ObjectSorting", robots="UR5e",
    ...
    # 테이블(씬 설정 참조) 커버리지: 카메라 높이에서 테이블면까지 거리 d일 때 시야폭 ≈ 2d·tan(fov/2)
