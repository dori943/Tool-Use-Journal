"""Experiment 1 v6 condition definitions (selective Stage-1 + OpenAI Stage-2).

Same C1-C7 factor matrix as v4/v5. Stage1 requests only the cues listed in
``siphy_cues`` (not full SiPhyBackend.estimate). ``gemini_infer`` names the
Stage2-inferred fields (kept for schema parity with v5).
"""
