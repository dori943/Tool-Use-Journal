...
def describe_pipeline(condition: Condition) -> dict[str, Any]:
    ...
    return {
        "stage1_model": "production_SiPhyBackend",
...
class Experiment1V4Runner:
    ...
    def run_unit(
        ...
        *,
        ...
        live_gemini_count = 1
        if self.dry_run:
            total_llm = live_siphy_count + live_gemini_count
            gemini_count_logged = 0
            siphy_count_logged = 0
        else:
            total_llm = siphy_calls + g_calls
        ...
        return UnitResult(
            ...
            total_llm_call_count=total_llm if not self.dry_run else 0,
