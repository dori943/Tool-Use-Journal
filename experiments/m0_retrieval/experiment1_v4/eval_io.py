"""v4 output / evaluation I/O — same semantics as Experiment 1 v3.

Reuses (import only; do not edit):
  - experiments/m0_retrieval/evaluator.py
  - experiments/m0_retrieval/result_writer.py

Writes under output/m0_retrieval/experiment1_v4/ (units/, raw_results*.csv,
...
from v4_conditions import CONDITIONS, PROPERTIES  # noqa: E402
...
# v3 columns + v4 stage token/call breakdown (common aliases kept).
V4_EXTRA_FIELDS = [
    "siphy_input_tokens",
    "siphy_output_tokens",
    "siphy_total_tokens",
...
RAW_FIELDS = list(V3_RAW_FIELDS) + [f for f in V4_EXTRA_FIELDS if f not in V3_RAW_FIELDS]
...
def recompute_from_units(
    *,
    ...
    if not units:
        print("[v4] No eligible units/ found to recompute (live_runs/ is ignored).")
        return 1
    ...
    write_json(
        out_dir / "run_metadata.json",
        {
            ...
            "note": "Aggregated from experiment1_v4/units/ only; live_runs/ ignored.",
        },
    )
    print(f"[v4] recomputed {len(units)} units → {len(all_rows)} raw rows")
...
def static_eval_pipeline_test() -> int:
    ...
    print("=" * 66)
    print("[v4] STATIC EVAL PIPELINE TEST")
