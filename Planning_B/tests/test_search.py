...
from planning_b.candidate_provider import StaticCandidateProvider, filter_candidates
from planning_b.conditions import (
    apply_effects,
    initial_facts,
...
from planning_b.feasibility import StaticFeasibilityChecker
from planning_b.models import PlanningBRequest, PlanningPolicy
from planning_b.planner import plan
from planning_b.state import SearchState, with_group_binding
from planning_b.transitions import (
    P,
    TransitionContext,
    build_terminal_transition,
    build_transition,
)
from planning_b.validation import build_predecessor_map, normalized_edges
...
def _diamond_request() -> PlanningBRequest:
    subgoals = [
...
def _brute_force_best_cost(request: PlanningBRequest) -> CostVector | None:
...
def _random_request(rng: random.Random) -> PlanningBRequest:
    n = rng.randint(3, 5)
    subgoals = []
    proposals = {}
    edges = []
