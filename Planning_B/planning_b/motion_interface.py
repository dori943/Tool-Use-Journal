"""Contract between Planning B and the motion planner.

Planning B never generates trajectories. It asks a motion-side oracle two kinds
of question:
...
Scene identity
--------------
Planning B does not own a geometric world model, so it cannot ship object poses
...
reconstructs a predicted scene by replaying that history. Because Planning B
...
planning run. Planning B caches by query identity and treats a cached answer as
...
Cost units
----------
:class:`~planning_b.cost.CostVector` is integer-valued on purpose: lexicographic
...
from planning_b.feasibility import CheckResult, FeasibilityStatus
...
class WorldSnapshot:
    ...
    Planning B passes through whatever the upstream scene graph supplied; it
...
class CandidateQuery:
    ...
    the externally supplied task pose. Planning B passes ``target_pose``
...
class TransitionQuery:
    ...
    ``primitives`` is the ordered primitive-action sequence Planning B intends
...
class EEExchangeQuery:
    ...
    Planning B currently validates rack slot *bookkeeping* symbolically but has
...
class MotionCostResult:
    ...
    Planning B then falls back to its configured nominal ``MotionCosts``.
    """
