...
class Subgoal(_AliasedModel):
    ...
    def _normalize_tool_source(self) -> "Subgoal":
        ...
        if legacy_candidates:
            raise ValueError(
                "Planning B does not accept tool candidates; provide the "
...
class OrderConstraints(_ExtensibleModel):
    ...
    # Diagnostic only. Planning B never enumerates topological orders.
...
class GraspSpec(_ExtensibleModel):
    ...
    Planning B treats ``pose`` and the optional approach/contact payloads as
...
class PlanningBRequest(_ExtensibleModel):
